...
describe('minimal C parser', () => {
    describe('literal string input', () => {
        it('should recognize variables as described by declList -> decl-> typeSpecificer ID', () => {
            var fn = function () { throw new Error("123"); }
        ...
        it('should recognize initializing arrays as described by declList -> decl -> typeSpecifier [INTCONST]', () => {
            expect(() => minCParser.parse("int x [5];")).to.to.not.throw('Parse error');
            expect(() => minCParser.parse("char x [5];")).to.to.not.throw('Parse error');
            expect(() => minCParser.parse("void x[5];")).to.to.not.throw('Parse error');
        }),
        it('handles comments', () => {
            expect(() => minCParser.parse("/*123123*/")).to.to.not.throw('Parse error');
        }), 
        describe('functions as described by declList -> funcDecl -> typeSpecifier () funBody', () => {
            it('should recognize function without localDeclList or statementList', () => {
                expect(() => minCParser.parse("void f () {}")).to.to.not.throw('Parse error');
            }),
            it('function with localDeclist and without statementList', () => {
                expect(() => minCParser.parse("void f () { int x; }")).to.to.not.throw('Parse error');
            }),
            it('function without localDeclList and with statementList (compoundStmt)', () => {
                expect(() => minCParser.parse("void foo () { {{ x = 2 + 2; }} {} }")).to.to.not.throw('Parse error');
            }),
            ...
            it('function without localDeclList and with statementList (condStmt)', () => {
                expect(() => minCParser.parse("void foo () { if(x == 2) x = 5; }")).to.to.not.throw('Parse error');
            }),
            it('function without localDeclList and with statementList (loopStmt)', () => {
                expect(() => minCParser.parse("void foo () { while(x == 2) x = 5; }")).to.to.not.throw('Parse error');
            }),
            it('function without localDeclList and with statementList (returnStmt)', () => {
                expect(() => minCParser.parse("void foo () { return; }")).to.to.not.throw('Parse error');
                expect(() => minCParser.parse("void foo () { return 2; }")).to.to.not.throw('Parse error');
            }),
            it('function with both localDeclList and statementList', () => {
                expect(() => minCParser.parse("void foo () { int x; x = 5; while(x == 2) x = 5; }")).to.to.not.throw('Parse error');
            }),
    ...
    describe('file input', () => {
        it('should recognize variables in a file', (done) => {
            readFile('pass/variables.mC', contents => {
                expect(() => minCParser.parse(contents)).to.to.not.throw('Parse error');
                done();
            })
        }),
