...
RegExpLexer.prototype = {
    ...
    validateNumber: function validateNumber(number) {
        ...
        return (regexPattern.test(number) || number === '0') 
            ? {INTCONST: parseInt(number)} : "Minimal C does not allow leading 0s";
    },

    validateChar: function validateChar(char) {
        var regexPattern = /'(\w)'/;
        var match = regexPattern.exec(char);
        if (match === null) {
            return "Character Pattern not found: " + char;
        }
        return {KWD_CHAR:match[1]}; // the first captured group is in the index 1
    },
    
    validateInvalidToken: function validateInvalidToken(string) {
        var generateError = function generateError(yylloc) {
            return "(" + yylloc.first_line + "," + yylloc.last_column + ")";
        };
        if (string[0] === "'") {
            return "Unclosed quote on character " + generateError(this.yylloc);
        }
        else if (string[0] === '"') {
            return "Unterminated string " + generateError(this.yylloc);
        }
        else if (string[0] === '/') {
            return "Unterminated comment " + generateError(this.yylloc);
        }
        return {ILLEGAL_TOK: string}
    },
    ...
    validateString: function validateString(string) {
        string = string.replace("\\t", "\t")
                       ...
                       .replace('\\"', '"')
                       .replace("\\\\", "\\");
        return {STRCONST: string.substring(1, string.length-1)};
    },
    
    validateComment: function validateComment(comment) {
        var validCommentPattern = /\/\*.*\*\//;
        if (validCommentPattern.test(comment)) {
            return 1;
        }
        return this.validateInvalidToken(comment);
    },
